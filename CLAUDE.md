...
- **Settle (TTL):** after a confirmed drop holds for **7 days** without recovery, the episode is marked `settled` (not recovered), the field returns to `idle` with the current price as the new anchor, and it leaves `active_drops`. Permanent catalog cuts close this way.
...
- `price-drops.json` is the **single UI source**: `active_drops`, `recovered_drops` (24h), `settled_drops` (24h), and `episodes` (full history). Banner counts **active** drops only.
