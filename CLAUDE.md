...
- **Time-of-day pricing** (`modelwatch/pricing_schedule.py`): OpenRouter's models API reports `pricing` for the window that is active **right now** and repeats the weekly schedule under `pricing.overrides`. Only entries carrying `utc_days`/`utc_start`/`utc_end` are schedules (HHMM UTC values, `0` = 24:00); entries keyed by `min_prompt_tokens` are long-context tiers and are ignored. The build derives `model.pricing_schedule` with `standard` (highest scheduled rate), `minimum` and labelled windows, and detection, history and the 7-day MA all use the **standard** rate — a model entering a peak or off-peak window is not a price change.
- **Two tracks per scheduled field**: the standard rate, plus an **off-peak track** (state key `prompt_offpeak`, …) holding the cheapest scheduled rate, so a deeper discount still raises an episode. Off-peak tracks have no history of their own and run without the MA gate, relying on the two-build settlement; episodes carry `tier` (`null` = standard, `"offpeak"`). A model that stops publishing a discount has its off-peak state pruned.
- **Basis-change rebase**: when a model's schedule appears or disappears relative to `previous.json` (`_comparison_basis_changed`), the build re-anchors it with `rebase_model_field_change_states` instead of reporting a change; in-flight pending/confirmed state is dropped and orphaned actives heal to `recovered`.
- **UI**: scheduled models show a price range (`$0.15–$0.30/M`) with a tier chip, local-time window labels, an `Off-peak now` pill, and `Prompt (off-peak)` labels on tiered episodes; the table sorts on the standard rate.
...
- **CI gates:** `build-and-deploy.yml` runs `pytest`, `mypy`, and `ruff` (plus web `lint`/`test`) in a `test-and-lint` job before build/deploy. CI types `modelwatch` **and** `tests` — run `uv run mypy modelwatch tests` locally to match.
